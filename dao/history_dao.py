...
class History_dao:
    ...
    def insert(self, user, trans_type, stock, price, total_price, volume, algo_id):
        self.db.query("insert into history (user, trans_type, stock, price, total_price, volume, algo_id) values ('%s','%s','%s','%s','%s','%s',%d, '%s')"%(user, trans_date, trans_type, stock, round(Decimal(price),2), round(Decimal(total_price),2), volume, algo_id)+";")
